...
class ChatInputViewModel extends ChangeNotifier {
  ...
  // تخزين الملفات المؤقتة بعد الرفع
  ...
  void sendPendingAttachments({
    ...
    try {
      ...
      final imageAttachments = _pendingAttachments.where(
              (a) => a.type?.startsWith("image/") == true
      ).toList();

      final audioAttachments = _pendingAttachments.where(
              (a) => a.type?.startsWith("audio/") == true
      ).toList();

      final fileAttachments = _pendingAttachments.where(
              (a) => a.type?.startsWith("image/") != true && a.type?.startsWith("audio/") != true
      ).toList();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("فشل في إرسال المرفقات: $error"),
