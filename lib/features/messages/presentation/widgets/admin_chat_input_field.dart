...
class _AdminChatInputFieldState extends State<AdminChatInputField> {
  ...
  void _handleAttachmentUpload(List<AttachmentModel> attachments) {
    ...
    for (final att in attachments) {
      ...
      //   receiverType: widget.receiverUserRole,
      //   onMessageSent: widget.onMessageSent,
      //   onUploadProgress: (id, progress) => _updatePendingMessageProgress(id, progress),
      //   onUploadSuccess: (id, fileUrl) {
      //     _updatePendingMessageWithFileUrl(id, fileUrl);
      //     Future.delayed(const Duration(seconds: 3), () => _removePendingMessage(id));
      //   },
      //   onUploadError: _updatePendingMessageError,
      // );
    }
